...
const fieldRows = [
    ...gameField.querySelectorAll(".field-row")
];
const fieldCells = fieldRows.map((row)=>[
        ...row.children
    ]);
 ...
 */ class Game {
   ...
   */ rotateCounterClockwise(transformedMatrix) {
    ...
    afterAction(newCellsCount = 1) {
        ...
        if (this.isLose(state)) this.status = statuses.lose;
    }
    validateState(localState) {
        if (this.status !== statuses.playing) return;
